      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white z-[56] transform transition-transform duration-500 flex flex-col ${open ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#E0E0E0]">
          <div className="flex items-center gap-2">
            ...
            {cart?.items?.length ? (
              <Badge variant="dark" className="text-[9px] px-1.5 py-0.5">{cart.items.reduce((n, i) => n + i.quantity, 0)}</Badge>
            ) : null}
          </div>
        ...
        <div className="flex-1 overflow-hidden">
          {!user ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-6">
              <ShoppingBag size={40} strokeWidth={1} className="text-[#E0E0E0]" />
                ...
                {cart.items.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="flex gap-4">
                      <div className="w-20 h-24 bg-[#F7F7F7] relative shrink-0 overflow-hidden">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between gap-2">
                          <Link href={`/shop/${item.product.slug}`} onClick={onClose}
                            className="font-medium text-sm leading-tight hover:opacity-60 transition-opacity line-clamp-2">
                            {item.product.name}
                          </Link>
        ...
        {/* Footer */}
        {cart?.items?.length ? (
          <div className="border-t border-[#E0E0E0] px-6 py-6 space-y-3">
            ...
            {parseFloat(cart.discount_total) > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <Badge variant="dark">−{sym}{parseFloat(cart.discount_total).toLocaleString()}</Badge>
              </div>
            )}
            <Separator />
            ...
            <Button asChild variant="ghost" size="sm" className="w-full text-[#8C8C8C]">
              <Link href="/shop" onClick={onClose}>Continue Shopping</Link>
            </Button>
          </div>
        ) : null}
